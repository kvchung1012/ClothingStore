...
namespace ClothesStore.WebApp.Areas.Admin.Controllers
{
    ...
    public class SliderController : Controller
    {
        ...
        public SliderController(ISliderService sliderService, IEmployeeService employeeService, IHttpContextAccessor HttpContextAccessor)
        {
        ...
        public async Task<PartialViewResult> GetFormAddOrEdit(int Id)
        {
            if (Id == 0)
                ...
                return PartialView(await _sliderService.GetObjectById(Id));
        }

        [HttpPost]
        public async Task<PartialViewResult> ViewDetail(int Id)
        {
            var obj = await _sliderService.GetObjectById(Id);
            ...
            var data = new ViewDetailObject<Slider>()
            {
                obj = obj,
        ...
        {
            var response = await _sliderService.GetListData(requestData);
        ...
        {
            var response = await _sliderService.GetObjectById(Id);
            return Json(response);
        }

        [HttpPost]
        public async Task<JsonResult> AddOrUpdate(Slider obj)
        {
            ...
            var response = await _sliderService.AddOrUpdate(obj);
        ...
        {
            var response = await _sliderService.DeleteById(Id);
            return Json(response);
        }
    }
}
